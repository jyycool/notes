...
1. ***text：即文本格式的可读数据***，我们最熟悉的应该就是 ***text/html 了，表示超文本文档***，此外还有***纯文本 text/plain***、***样式表 text/css*** 等。
...
有了 MIME type 和 Encoding type，无论是浏览器还是服务器就都可以轻松识别出 body 的类型，也就能够正确处理数据了。H

TTP 协议为此定义了两个 Accept 请求头字段和两个 Content 实体头字段，用于客户端和服务器进行“内容协商”。也就是说，***客户端用 Accept 头告诉服务器希望接收什么样的数据，而服务器用 Content 头告诉客户端实际发送了什么样的数据。***

![body_mime_编码](/Users/sherlock/Desktop/notes/allPics/通信协议/body_mime_编码.png)
...
相应的，服务器会在响应报文里用头字段 Content-Type 告诉实体数据的真实类型 (***Content-type是实体字段，请求响应里都可以出现*** )：
...
Accept-Language 字段标记了客户端可理解的自然语言，也允许用“,”做分隔符多个类型，例如：
...
![数据和语言类型及编码](/Users/sherlock/Desktop/notes/allPics/通信协议/数据和语言类型及编码.png)
...
```
http://www.cy.com/15-1?name=a.json
http://www.cy.com/15-1?name=a.xml
```

在 Chrome 里打开开发者工具，就能够看到 Accept 和 Content 头：

![http_lang_test](/Users/sherlock/Desktop/notes/allPics/通信协议/http_lang_test.png)
...
![数据及语言](/Users/sherlock/Desktop/notes/allPics/通信协议/数据及语言.png)
...
> 作业: 假设你要使用 POST 方法向服务器提交一些 JSON 格式的数据，里面包含有中文，请求头应该是什么样子的呢？
>
> POST / HTTP/1.1
>
...
不过这个解决方法也有个***缺点，gzip 等压缩算法通常只对文本文件有较好的压缩率，而图片、音频视频等多媒体数据本身就已经是高度压缩的，再用 gzip 处理也不会变小（甚至还有可能会增大一点），所以它就失效了。***
...
![chuncked](/Users/sherlock/Desktop/notes/allPics/通信协议/chuncked.png)

实验环境里的 URI“/16-1”简单地模拟了分块传输，可以用 Chrome 访问这个地址看一下效果：

![test_chunked](/Users/sherlock/Desktop/notes/allPics/通信协议/test_chunked.png)

不过浏览器在收到分块传输的数据后会自动按照规则去掉分块编码，重新组装出内容，所以想要看到服务器发出的原始报文形态就得用 Telnet 手工发送请求（或者用 Wireshark 抓包）：

因为 Telnet 只是收到响应报文就完事了，不会解析分块数据，所以可以很清楚地看到响应报文里的 chunked 数据格式：先是一行 16 进制长度 f，然后是数据，然后再是 16 进制长度 f和数据，如此重复，最后是 0 长度分块结束。
...
Date: Thu, 06 Aug 2020 05:24:15 GMT
...
Server: openresty/1.17.8.2
Date: Thu, 06 Aug 2020 05:29:25 GMT
Content-Type: text/plain
Content-Length: 32
...
![分段数据](/Users/sherlock/Desktop/notes/allPics/通信协议/分段数据.png)
...
HTTP/1.1 206 Partial Content
Server: openresty/1.17.8.2
Date: Thu, 06 Aug 2020 05:35:36 GMT
...
报文里的“--00000000000000000001”就是多段的分隔符，使用它客户端就可以很容易地区分出多段 Range 数据。
...
![短连接](/Users/sherlock/Desktop/notes/allPics/通信协议/短连接.png)
...
![长连接](/Users/sherlock/Desktop/notes/allPics/通信协议/长连接.png)
...
![连接响应头](/Users/sherlock/Desktop/notes/allPics/通信协议/连接响应头.png)
...
![连接关闭](/Users/sherlock/Desktop/notes/allPics/通信协议/连接关闭.png)
...
![队头阻塞](/Users/sherlock/Desktop/notes/allPics/通信协议/队头阻塞.png)
...
> 问题1: 域名分片后面的服务器还是那个服务器, 既是多开了 N 个域名, 连接还是指向该台服务器, 这是如何解决队头阻塞
>
> 答: 其实域名分片本质还是为了提高 并发连接, 因为浏览器会限制一个域名最多 6 个连接, 那么 把域名分片数变成 3, 这样 3 x 6 =18 个连接还是指向了目标服务, 但是并发的长连接数却扩增了.
>
> 问题2: 高并发请求和并发连接有什么关系吗？负载均衡解决高并发问题是并发连接吗？
>
> 答: “高并发请求”是服务器端的概念，意思是同时有多个客户端连接服务器。“并发连接”是客户端的概念，意思是一个浏览器并发多个连接，访问服务器。负载均衡是服务器端的概念，就是把大量的客户端连接均匀分散到集群里的多台服务器。
...
这样的跳转动作是由浏览器的使用者主动发起的，可以称为“主动跳转”，但***还有一类跳转是由服务器来发起的，浏览器使用者无法控制，相对地就可以称为“被动跳转”，这在 HTTP 协议里有个专门的名词，叫做“重定向”（Redirection）***。
...
例如，刚才的实验例子里的“Location: /index.html”用的就是相对 URI。它没有说明访问 URI 的协议和主机，但因为是由“http://www.cy.com/18-1”重定向返回的响应报文，所以浏览器就可以拼出完整的 URI：

```
http://www.cy.com/index.html
```

实验环境的 URI“/18-1”还支持使用 query 参数“dst=xxx”，指明重定向的 URI，你可以用这种形式再多试几次重定向，看看浏览器是如何工作的。

```
http://www.cy.com/18-1?dst=/15-1?name=a.json
http://www.cy.com/18-1?dst=/17-1
```

注意，在重定向时如果只是在站内跳转，你可以放心地使用相对 URI。但如果要跳转到站外，就必须用绝对 URI。

例如，如果想跳转到 Nginx 官网，就必须在“nginx.org”前把“http://”都写出来，否则浏览器会按照相对 URI 去理解，得到的就会是一个不存在的 URI“http://www.cy.com/nginx.org”

```
http://www.cy.com/18-1?dst=nginx.org 				#错误
http://www.cy.com/18-1?dst=http://nginx.org #正确
```
...
![18-2-无访问](/Users/sherlock/Desktop/notes/allPics/通信协议/18-2-无访问.png)
...
![Cookie 执行过程](/Users/sherlock/Desktop/notes/allPics/通信协议/Cookie 执行过程.png)
...
在实验环境里，你可以用 Chrome 访问 URI“/19-1”，实地看一下 Cookie 工作过程。首次访问时服务器会设置两个 Cookie。

![首次 cookie](/Users/sherlock/Desktop/notes/allPics/通信协议/首次 cookie.png)

然后刷新这个页面，浏览器就会在请求头里自动送出 Cookie，服务器就能认出你了。

![刷新后 cookie](/Users/sherlock/Desktop/notes/allPics/通信协议/刷新后 cookie.png)
...
![cookie 属性](/Users/sherlock/Desktop/notes/allPics/通信协议/cookie 属性.png)
...
![all-cookies](/Users/sherlock/Desktop/notes/allPics/通信协议/all-cookies.png)
...
> 问: 如果 Cookie 的 Max-Age 属性设置为 0，会有什么效果呢？
>
> 答: RFC 里有说明，如果max-age <=0，统一按0算，立即过期。
...
![http 缓存](/Users/sherlock/Desktop/notes/allPics/通信协议/http 缓存.png)

你可以访问实验环境的 URI “/20-1”，看看具体的请求 - 应答过程。

![http_cache_exp](/Users/sherlock/Desktop/notes/allPics/通信协议/http_cache_exp.png)

服务器标记资源有效期使用的头字段是“Cache-Control”，里面的值“max-age=30”就是资源的有效时间，相当于告诉浏览器，“这个页面只能缓存 30 秒，之后就算是过期，不能用。

”你可能要问了，让浏览器直接缓存数据就好了，为什么要加个有效期呢？这是因为网络上的数据随时都在变化，不能保证它稍后的一段时间还是原来的样子。就像生鲜超市给你快递的西瓜，只有 5 天的保鲜期，过了这个期限最好还是别吃，不然可能会闹肚子。
...
> 注意这里说的是: 在响应报文里可以使用其他属性来精确指示浏览器如何使用缓存, 所以关于缓存的一些策略都是从服务器发出来的.
>
> 问: 那么浏览器自己可以指定缓存的策略么? 
>
> 答: 
...
![http_缓存策略流程图](/Users/sherlock/Desktop/notes/allPics/通信协议/http_缓存策略流程图.png)
...
它的 Request Headers 中其实增加了个header: “Cache-Control: no-cache”，含义和“Cache-Control: max-age=0”基本一样，就看后台的服务器怎么理解，通常两者的效果是相同的。
...
另外，如果用第 18 讲里的重定向跳转功能，也可以发现浏览器使用了缓存：http://www.cy.com/18-1?dst=20-1

![http_cache_exp2](/Users/sherlock/Desktop/notes/allPics/通信协议/http_cache_exp2.png)
...
![modified](/Users/sherlock/Desktop/notes/allPics/通信协议/modified.png)

“Last-modified”很好理解，就是文件的最后修改时间。ETag 是什么呢？ETag 是“实体标签”（Entity Tag）的缩写，是资源的一个唯一标识，主要是用来解决修改时间无法准确区分文件变化的问题。
...
![etag](/Users/sherlock/Desktop/notes/allPics/通信协议/etag.png)
